...
- **Small diffs, recorded rules.** 84% of diffs over 1,000 lines draw review findings, against 31% under 50 — hence the five-source-file cap per task. Codifying accepted review comments as rules, plus a 15-item pre-submission self-review checklist, eliminated recurrences across nine error classes — hence the checklist every implementer and fixer answers against its diff, and the reviewer verifies.
